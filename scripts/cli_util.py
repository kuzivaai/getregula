...
def cmd_verify(args) -> None:
    ...
    # A v1.1 manifest may embed an Ed25519 signature over the canonical
    # (signing-field-stripped) manifest JSON. Behaviour:
    #   - No signing block present: unsigned pack. Warn if --strict, else silent.
    #   - Signing block present + signature valid: report confirmed.
    #   - Signing block present + signature invalid: fail regardless of --strict.
    #   - Signing block present + `cryptography` not installed: warn; fail under
    #     --strict (cannot verify the tamper claim).
    ...
    if signing_block_present:
        try:
            ...
            try:
                ...
                if ok:
                    ...
                    signature_detail = detail
                    # Invalid signatures always fail, regardless of strict.
        ...
        # Unsigned manifest. Under strict we note (but don't fail) — the
        # v1 spec makes signing optional even in v1.1.
        if strict:
            warnings.append(
