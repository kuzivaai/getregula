...
The real fix for a reader who cannot read English is a translated page. That
is a content and commercial decision, and `PRODUCT_BUILD` is STOP and
`PAYMENT_GATE` is NOT_ACTIVE, so it is not made here. Pricing in particular is
the most claim-sensitive surface in the repository: the two commits before this
work were both price-integrity fixes, and duplicating prices into two more
locales would triple the drift surface under a payment freeze.
