...
# is not in the determination class  closed. The owner ruled on
...
def scan_dependencies(project_path: Path) -> dict:
    ...
    for dep_file, language in DEPENDENCY_FILES.items():
        ...
        # A refused read (escaping symlink, FIFO, oversized) must SKIP the
        # file, matching the original `except OSError: continue`. Coercing
        # it to "" instead would silently treat a rejected dependency file
        # as an empty one — the same content, a different meaning.
        _content = read_text_if_safe(filepath, errors="ignore")
        if _content is None:
            continue  # dep file unreadable or refused by the guard; skip
...
def generate_eu_registration(project_name: str) -> dict:
    ...
    Maintained for backwards compatibility with callers that imported this
    function directly. Delegates to register.build_packet under the hood and
    re-shapes the result to the legacy key set.
    """
    registry = load_registry()
    ...
    from register import build_packet  # local import to avoid circular dep at module load

    discovery_like = {
        "project_name": project_name,
        "project_path": system.get("project_path", ""),
        "compliance_status": system.get("compliance_status", "not_started"),
        "ai_libraries": system.get("ai_libraries", []),
        "model_files": system.get("model_files", []),
        "highest_risk": system.get("highest_risk", "minimal_risk"),
        "risk_classifications": system.get("risk_classifications", []),
    }

    # Legacy callers always wanted the Section A / Art 49(1) view — point 4 (employment)
    # is the safe default since it routes to Section A / public / Art 49(1) without exclusions.
    packet = build_packet(
        discovery=discovery_like, role="provider", annex_iii_point=4,
    ...
    return {
        ...
        "article": packet.get("article", "49(1)"),
        ...
        "note": "Auto-generated by Regula. Prefer `regula register` for the full Annex VIII packet.",
        "_packet": packet,
    }
...
def main():
    ...
    if args.compliance:
        ...
        if not status:
            ...
            # correction in cli_compliance.py `workflow`. .
