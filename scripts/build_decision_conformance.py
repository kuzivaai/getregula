...
SHARD_DIR = ROOT / "references" / "decision_conformance.v1"
...
def _render_shard(model_version, shard_index, vectors):
    return _render_json({
        "schema_version": "1.0",
...
def render_bundle():
    """Return a scan-safe manifest and deterministic conformance shards."""
    corpus = build_corpus()
    vector_shards = []
    current = []
    for vector in corpus["vectors"]:
        candidate = current + [vector]
        rendered = _render_shard(
            corpus["model_version"], len(vector_shards), candidate
        )
        if current and len(rendered.encode("utf-8")) > MAX_SHARD_BYTES:
            vector_shards.append(current)
            current = [vector]
        else:
            current = candidate
    if current:
    ...
    rendered_shards = {}
    shard_entries = []
    for index, vectors in enumerate(vector_shards):
        relative_path = f"decision_conformance.v1/part-{index:03d}.json"
        rendered = _render_shard(corpus["model_version"], index, vectors)
        encoded = rendered.encode("utf-8")
        if len(encoded) > MAX_SHARD_BYTES:
            raise ValueError(
                f"conformance shard {relative_path} exceeds {MAX_SHARD_BYTES} bytes"
            )
        rendered_shards[relative_path] = rendered
    ...
    manifest = {key: value for key, value in corpus.items() if key != "vectors"}
    manifest["storage"] = {
        "format": "sharded-v1",
        "max_shard_bytes": MAX_SHARD_BYTES,
    }
    manifest["shards"] = shard_entries
    return _render_json(manifest), rendered_shards


def render():
    """Render the manifest; retained for callers that only check that file."""
    return render_bundle()[0]


def main():
    manifest_text, shards = render_bundle()
    SHARD_DIR.mkdir(parents=True, exist_ok=True)
    expected_paths = {ROOT / "references" / path for path in shards}
    for relative_path, rendered in shards.items():
        path = ROOT / "references" / relative_path
        path.write_text(rendered, encoding="utf-8")
    for stale_path in SHARD_DIR.glob("part-*.json"):
        if stale_path not in expected_paths:
            stale_path.unlink()
    TARGET.write_text(manifest_text, encoding="utf-8")
    manifest = json.loads(manifest_text)
    print(json.dumps(manifest["counts"], indent=2, sort_keys=True))
    print(f"wrote {len(shards)} shards in {SHARD_DIR.relative_to(ROOT)}")
