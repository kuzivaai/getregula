...
rule violation, not a risk.** See `docs/TRUST.md` 4c and 4d.
...
# tests/test_classification.py, which AGENTS.md requires.
#
...
# already wrong by one at `5871199`. It is the exact shape measurement rule 5
...
#
# WHY IT IS HERE. the prior audit recorded it as a fourth ungated quantity and it is
...
# and simultaneously the prior audit's (the unit word is not always adjacent). Both
...
def canonical_count() -> int:
    ...
    That matters beyond this tool: rc=0 can be read as proof that published
    counts are current, so the live computation must be part of the check.
