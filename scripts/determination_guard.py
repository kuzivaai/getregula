...
CLAUDE.md states the project's hard rule twice: never present a scan result as a
compliance determination, and never present "not flagged" as "compliant". On
2026-08-15 a dossier pass found three shipped paths doing exactly that
(LEDGER N125), the worst being `regula badge`, which emitted a pasteable
...
EXCLUDED_PREFIXES = (
    "docs/improvement/",      # session records and this programme's own ledger
    "CHANGELOG.md",           # records what shipped, verbatim
    "docs/venture/",          # dated evidence registers, frozen at capture
...
def run(argv=None) -> int:
    ...
    if findings or stale:
        print("\nAn affirmative compliance-state assertion is the one claim this "
              "project forbids outright (CLAUDE.md, LEDGER N125). Re-express it "
