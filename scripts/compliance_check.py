...
def _is_generated_draft(abs_path: str) -> bool:
    """True if the file is an unfilled Regula-generated scaffold.

    Auto-generated drafts must not count as compliance evidence — the
    July 2026 audit found `regula gap` scoring this repo 100% STRONG on
    the back of its own tmp annex-IV drafts, while `regula check` on the
    same tree said PROHIBITED. Evidence has to be something a human
    maintained, not something Regula emitted a moment ago.
    """
    # Bare open() blocks forever on a FIFO — a third read form the earlier
    # read_text sweep never covered, and the last command still hanging.
    head = read_text_if_safe(Path(abs_path), errors="ignore")
    if head is None:
        return False
    head = head[:600]
    return "Generated by Regula" in head


def _walk_project(project_path: str):
    """Yield (relative_path, absolute_path) for all scannable files.

    Skips hidden directories (tool/config trees like .claude, .idea are
    not project compliance documentation) except .regula, whose registry
    the Article 49 checker reads. Skips Regula-generated draft scaffolds
    (see _is_generated_draft).
    """
    ...
    for root, dirs, files, _dirfd in _walk:
        dirs[:] = [
            d for d in dirs
            if d not in SKIP_DIRS and (not d.startswith(".") or d == ".regula")
        ]
        for filename in files:
            filepath = Path(root) / filename
            if filepath.suffix.lower() in ALL_SCANNABLE:
                if _is_generated_draft(str(filepath)):
                    continue
                try:
    ...
    for rel_path, abs_path in files_index:
        ...
        if _file_matches_glob(rel_path, tech_doc_file_patterns):
            ...
            if "annex" in basename_lower:
                ...
                try:
                    card_content = read_text_if_safe(filepath, errors="ignore")
...
def format_gap_text(assessment: dict) -> str:
    ...
    lines.append("")
    lines.append("  NOTE: This score measures the PRESENCE of compliance")
    lines.append("  documentation and infrastructure — it does not assess code")
    lines.append("  risk and cannot offset scan findings. A project can score")
    lines.append("  100% here and still fail `regula check` on prohibited or")
    lines.append("  high-risk patterns. Run both; they answer different questions.")
    lines.append("")
