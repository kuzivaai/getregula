...
def ref_is_tracked(ref: str) -> bool:
    ...
    commit. `docs/TRUST.md` rule 4b already holds that an
...
def unscannable_delivery_surfaces() -> dict[str, list[str]]:
    ...
    MEASURED 2026-08-17 at `ba61911`: 6 of 108. `main` filtered its reports with
...
# a heading reading "Honest baselines (measured, reproducible)". the prior audit.
...
# published ones plus the benchmark writeups that feed them.
#
...
# published two figures the prior audit had withdrawn. `recall_checked_files()`
...
def recall_checked_files() -> list[str]:
    ...
    inventory classifies only active, claim-capable delivery surfaces, so
    historical records stay out of scope without a hand-maintained directory
    exclusion.
    """
...
def check_recall_percentages(text: str, rel_path: str) -> list[tuple[int, str]]:
    ...
    other form the same claim is written in. the prior audit: the fraction-only
...
def verify_facts() -> int:
    ...
    for rel_path in recall_files:
        ...
        # the prior audit: the fraction-only arm passed two surfaces that were
