...
| F3 | **Send IAPP email (D5)** | 5 min | Yes (draft in HUMAN_ACTIONS §7) | The IAPP AI Governance Vendor Report is the primary discovery channel for compliance buyers (CTOs, DPOs, compliance leads). Regula is not listed. VERIFIED — IAPP report reviewed in Phase 0. |
| F4 | **Bing Webmaster Tools (D7)** | 15 min | Yes (steps in HUMAN_ACTIONS §1) | ChatGPT web search uses Bing's index. Without Bing indexing, getregula.com may be absent from ChatGPT search results entirely. VERIFIED — documented in OpenAI/Bing partnership. |
