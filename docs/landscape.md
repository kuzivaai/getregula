...
| Has a complaints mechanism for AI issues | [PDF only — unverified] | **No** | Operational mechanism, not a code property. |
...
- **Article 5 prohibited practice detection** — pattern-based detection of subliminal manipulation, social scoring, criminal-prediction-by-profiling, untargeted facial scraping, and 4 other Article 5 categories. Fires regardless of whether the file imports an AI library (fixed in commit `fd1910c`). Synthetic-fixture precision and recall are 100/100 against a hand-crafted ground-truth corpus.
