...
| 2,452 unique tests (2,452 pytest-collected), 6 self-tests, 0 known security findings | see [§3](#3-reproducibility) |
...
### 3.1 Internal test suite — 2,452 [unique](../tests/) / 2,452 pytest-collected, all green

```bash
git clone https://github.com/kuzivaai/getregula.git
cd getregula
python3 -m pytest tests/ -q
# Expected: 2452 passed. Wall-clock is machine-dependent: four runs on one
# laptop on 2026-07-29 spanned 11 to 17 minutes, so read it as minutes, not
# as a figure. The 2452 is the claim; the duration is not.
# 2,452 unique tests (sort -u of test IDs equals collected count).
```
...
| Test suite | `tests/` (2,452 unique tests, 2,452 pytest-collected; the legacy `tests/test_classification.py` runner executes 963 functions, 437 defined in-file) |
...
| Test suite | `tests/` | 2,452 unique tests (2,452 pytest-collected) |
