...
| 2,568 unique tests (2,568 pytest-collected), 6 self-tests, 0 known security findings | see [§3](#3-reproducibility) |
...
### 3.1 Internal test suite — 2,568 [unique](../tests/) / 2,568 pytest-collected, all green

```bash
git clone https://github.com/kuzivaai/getregula.git
cd getregula
python3 -m pytest tests/ -q
# Expected: 2568 passed. Wall-clock is machine-dependent and is NOT a claim;
# it has varied by a factor of two on one laptop in a single day. Quote the
# count, never the duration.
# 2,568 unique tests (sort -u of test IDs equals collected count).
```
...
(`Results: 1386 passed, 0 failed, 0 skipped (1023 test functions)` — verified
...
978, 1010, 1011, 1015 and 1023 functions**: 45 functions were added across those runs
and not one of them uses the helpers. **The number to read is the function count
...
| Test suite | `tests/` (2,568 unique tests, 2,568 pytest-collected; the legacy `tests/test_classification.py` runner executes 1,023 functions, 437 defined in-file) |
...
| Test suite | `tests/` | 2,568 unique tests (2,568 pytest-collected) |
