...
| 2,622 unique tests (2,622 pytest-collected), 6 self-tests, 0 known security findings | see [§3](#3-reproducibility) |
...
### 3.1 Internal test suite — 2,622 [unique](../tests/) / 2,622 pytest-collected, all green

```bash
git clone https://github.com/kuzivaai/getregula.git
cd getregula
python3 -m pytest tests/ -q
# Expected: 2622 passed. Wall-clock is machine-dependent and is NOT a claim;
# it has varied by a factor of two on one laptop in a single day. Quote the
# count, never the duration.
# 2,622 unique tests (sort -u of test IDs equals collected count).
```
...
| Test suite | `tests/` (2,622 unique tests, 2,622 pytest-collected; the legacy `tests/test_classification.py` runner executes 1,059 functions, 437 defined in-file) |
...
| Test suite | `tests/` | 2,622 unique tests (2,622 pytest-collected) |
