...
| 2,490 unique tests (2,490 pytest-collected), 6 self-tests, 0 known security findings | see [§3](#3-reproducibility) |
...
### 3.1 Internal test suite — 2,490 [unique](../tests/) / 2,490 pytest-collected, all green

```bash
git clone https://github.com/kuzivaai/getregula.git
cd getregula
python3 -m pytest tests/ -q
# Expected: 2490 passed. Wall-clock is machine-dependent and is NOT a claim;
# it has varied by a factor of two on one laptop in a single day. Quote the
# count, never the duration.
# 2,490 unique tests (sort -u of test IDs equals collected count).
```
...
(`Results: 1386 passed, 0 failed, 0 skipped (978 test functions)` — verified
2026-07-29). It walks `globals()` of `tests/test_classification.py`,
...
| Test suite | `tests/` (2,490 unique tests, 2,490 pytest-collected; the legacy `tests/test_classification.py` runner executes 978 functions, 437 defined in-file) |
...
| Test suite | `tests/` | 2,490 unique tests (2,490 pytest-collected) |
