...
| 2,967 unique tests (2,967 pytest-collected), 6 self-tests; versioned open-alert inventory retained | see [§3](#3-reproducibility) and [SECURITY.md](../SECURITY.md) |
...
### 3.1 Internal test suite — 2,967 [unique](../tests/) / 2,967 pytest-collected cases

```bash
git clone https://github.com/kuzivaai/getregula.git
cd getregula
python3 -m pytest tests/ --collect-only -q
# Expected: 2967 collected. This command measures collection only.
# 2,967 unique tests (sort -u of test IDs equals collected count).
...
The runner currently discovers 1,292 functions, a count machine-checked by
...
finds every `test_*` function, and executes it; 442 of those functions
...
| Test suite | `tests/` (2,967 unique tests, 2,967 pytest-collected; the legacy `tests/test_classification.py` runner executes 1,292 functions, 442 defined in-file) |
...
| Test suite | `tests/` | 2,967 unique tests (2,967 pytest-collected) |
