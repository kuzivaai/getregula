...
| 2,528 unique tests (2,528 pytest-collected), 6 self-tests, 0 known security findings | see [§3](#3-reproducibility) |
...
### 3.1 Internal test suite — 2,528 [unique](../tests/) / 2,528 pytest-collected, all green

```bash
git clone https://github.com/kuzivaai/getregula.git
cd getregula
python3 -m pytest tests/ -q
# Expected: 2528 passed. Wall-clock is machine-dependent and is NOT a claim;
# it has varied by a factor of two on one laptop in a single day. Quote the
# count, never the duration.
# 2,528 unique tests (sort -u of test IDs equals collected count).
```
...
(`Results: 1386 passed, 0 failed, 0 skipped (1010 test functions)` — verified
2026-07-30). **Read that line carefully: `1386 passed` is not a count of
tests.** The runner's counter is incremented by the `assert_true` / `assert_eq`
/ `assert_false` helpers in `tests/helpers.py`, so it counts helper assertions,
and a test written with a bare `assert` contributes nothing to it. The figure
was **also 1386 when the runner executed 978 functions**: 32 functions were
added between the two runs and none of them uses the helpers. **The number to
read is the function count in brackets.** It walks `globals()` of `tests/test_classification.py`,
...
| Test suite | `tests/` (2,528 unique tests, 2,528 pytest-collected; the legacy `tests/test_classification.py` runner executes 1,010 functions, 437 defined in-file) |
...
| Test suite | `tests/` | 2,528 unique tests (2,528 pytest-collected) |
