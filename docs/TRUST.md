...
| 3,098 unique tests (3,098 pytest-collected), 6 self-tests; versioned open-alert inventory retained | see [§3](#3-reproducibility) and [SECURITY.md](../SECURITY.md) |
...
### 3.1 Internal test suite — 3,098 [unique](../tests/) / 3,098 pytest-collected cases

```bash
git clone https://github.com/kuzivaai/getregula.git
cd getregula
python3 -m pytest tests/ --collect-only -q
# Expected: 3098 collected. This command measures collection only.
# 3,098 unique tests (sort -u of test IDs equals collected count).
...
The runner currently discovers 1,391 functions, a count machine-checked by
...
| Test suite | `tests/` (3,098 unique tests, 3,098 pytest-collected; the legacy `tests/test_classification.py` runner executes 1,391 functions, 442 defined in-file) |
...
| Test suite | `tests/` | 3,098 unique tests (3,098 pytest-collected) |
