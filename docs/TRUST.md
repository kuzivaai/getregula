...
| 2,936 unique tests (2,936 pytest-collected), 6 self-tests; versioned open-alert inventory retained | see [§3](#3-reproducibility) and [SECURITY.md](../SECURITY.md) |
...
>
> The landing-page demo is generated from a tracked-only snapshot of a
> committed fixture. Ignored or untracked files inside a local fixture cannot
> influence it. `tests/test_gap_demo.py` re-runs the commands and compares the
> output with `data/gap_demo.json`.

### 3.1 Internal test suite — 2,936 [unique](../tests/) / 2,936 pytest-collected cases

```bash
git clone https://github.com/kuzivaai/getregula.git
cd getregula
python3 -m pytest tests/ --collect-only -q
# Expected: 2936 collected. This command measures collection only.
# 2,936 unique tests (sort -u of test IDs equals collected count).
...
The runner currently discovers 1,301 functions, a count machine-checked by
...
finds every `test_*` function, and executes it; 444 of those functions
...
# NOTE: this command re-displays the tracked scored artefact
# (benchmarks/results/random_corpus/PRECISION.json); it does not
# re-derive the figure. The per-repo scans behind the 115-entry subset
# are untracked and the corpus clones were unpinned, so 83.5% is a
# dated April 2026 measurement. See "Methodology details and
# reproducibility" in benchmarks/README.md.
...
Default-branch CodeQL analysis `1646686319` at main commit `fe1f5e7` produced
...
| Test suite | `tests/` (2,936 unique tests, 2,936 pytest-collected; the legacy `tests/test_classification.py` runner executes 1,301 functions, 444 defined in-file) |
...
| Test suite | `tests/` | 2,936 unique tests (2,936 pytest-collected) |
