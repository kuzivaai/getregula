...
| 2,685 unique tests (2,685 pytest-collected), 6 self-tests; versioned open-alert inventory retained | see [§3](#3-reproducibility) and [SECURITY.md](../SECURITY.md) |
...
### 3.1 Internal test suite — 2,685 [unique](../tests/) / 2,685 pytest-collected, all green

```bash
git clone https://github.com/kuzivaai/getregula.git
cd getregula
python3 -m pytest tests/ -q
# Expected: 2685 passed. Wall-clock is machine-dependent and is NOT a claim;
# it has varied by a factor of two on one laptop in a single day. Quote the
# count, never the duration.
# 2,685 unique tests (sort -u of test IDs equals collected count).
```
...
| Test suite | `tests/` | 2,685 unique tests (2,685 pytest-collected) |
