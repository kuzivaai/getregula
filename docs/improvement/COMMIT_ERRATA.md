...
## Erratum 5: `e0e1864` states a commit count that its own landing changed (17 Aug 2026)

**Commit:** `e0e1864` "docs(ledger,merge): the branch has an open pull request,
...
is inside the count.** 40 and 38 were correct at `ad5706e`, the parent, which is
...
the tree it was measured in. Had the message said "measured at `ad5706e`", it
would have been correct and would not have needed this entry.

**Consequence.** Low. The load-bearing claims of `e0e1864` are the three
...
`238d1f1` is unpushed.* That formulation cannot go stale as further commits land,
