# Regula direct-transaction discovery launch-gate handover

## 1. Objective and actual work

Prepared and adversarially reviewed the smallest transaction-first human-discovery launch gate. No contact, real-data collection, product work, public change, spend, ownership action or immigration action occurred.

## 2. Sandbox, approval, network and Git state

Workspace and working directory: `/home/USER/getregula`. Workspace-write sandbox; read-only public network retrieval used. The escalated `.git` probe returned `git_dir_writable=yes`. No active concurrent writer was found. Local commits were authorised; no push was performed.

## 3. Initial repository state

Initial HEAD `799ce3d021a819e4584008839ede4c47fd55c2e1`; tree `ab912efbdc37d694ac52b2f0ab2e3c6b72946724`; branch `audit/regulatory-current-2026-08-04`; clean and 13 ahead of `origin/main`.

## 4. H1 and continuity verification

N67 continues to abandon the exact UK general-insurer claims-triage hypothesis as `GENERAL_PROBLEM_ONLY`. N66 remains partial and refers to N67. H1 was not reopened. H2 was not created. All commercial and product STOP states remain.

## 5. Existing readiness-pack audit

The existing 29-file preparation pack was reused. Participant information, consent/confidentiality, no-recording pathway, withdrawal/deletion process, data-receipt controls, owner permissions and gate remain useful drafts. They do not settle controller, lawful basis, storage, retention, permissions, participant access or legality. The former H1-specific discovery protocol remains historical and was not relabelled.

## 6. Primary and peer-reviewed sources

The complete source table, URLs/DOIs, dates, decisive passages, uses and limitations is [SOURCES.md](../venture/direct-discovery-2026-08-05/SOURCES.md). It covers current ICO/MRS sources, CIT and reviews, information power, empirical sample guidance, SRQR, COREQ and critiques, dyadic research, negative-case analysis and reflexivity. These sources support method choices only.

## 7. Sources unavailable or unresolved

No load-bearing source was substituted from memory. ICO guidance is explicitly under review. No source determines Regula demand, WTP, legal basis or sample adequacy for future unseen data.

## 8. Owner and professional decision register

The controlling register is [05-OWNER-AND-PROFESSIONAL-DECISIONS.md](../venture/direct-discovery-2026-08-05/05-OWNER-AND-PROFESSIONAL-DECISIONS.md). Controller, purpose, lawful basis, storage/system/jurisdiction/processors, access roles, retention, deletion/objection, privacy-notice ownership, Stage A prohibitions, cost cap, interviewer identity/work permission and research-sales separation remain required decisions.

## 9. Transaction-qualified population

Only first-hand participation within 12 months in a real active enterprise AI sale, procurement or onboarding review qualifies. Hypothetical, hearsay, marketing-only and confidentiality-dependent accounts are excluded. Insurance and other beachheads are not preselected.

## 10. Organisation-level access map

The existing map was retained as an access hypothesis. It contains organisation and role archetypes, not names, personal contacts or inferred personal attributes. No access or willingness is claimed.

## 11. Public retrieval attempts and limits

Retrieval was limited to the methodological sources needed to justify changed conventions. Public organisation scraping was not expanded because the existing organisation-level map was sufficient for owner review and no contact was authorised.

## 12. Event-based screener

The method requires side, direct role, recency, AI product, buyer type, stage, review type, first-hand involvement and non-confidential discussability. It rejects hypothetical, hearsay, generic-opinion and pitch accounts.

## 13. Supplier interview guide

Starts with the most recent qualifying transaction and captures trigger, evidence, roles, time/rework/delay/outcome, substitutes, buyer feedback, observability, constraints, actual expenditure and evidence making a tool unnecessary. No Regula priming or buying question is permitted.

## 14. Buyer interview guide

Captures mandatory/risk-based/negotiable requirements, requester/reviewer/blocker/approver, weak evidence, follow-up, independent review, source-code relevance, current systems, acceptance and cases where the process worked well.

## 15. De-identified transaction schema

The schema prohibits names, contacts, employer-identifying text, customer/product names, contracts, repositories, code, vulnerabilities and special-category data. It uses anonymous transaction/account/event IDs, explicit evidence states, roles, observability and independent-review fields. Only two synthetic examples exist.

## 16. Negative-case design

Future recruitment deliberately seeks fast passes, failures/delays, satisfied buyers, independent-assurance requirements, adequate incumbents, immaterial code, no dedicated budget and supplier refusal. Negative cases must be retained.

## 17. Sequential information-power plan

The 2 supplier + 2 buyer/reviewer, 2-organisation Stage A denominator is an access/method checkpoint only. Subsequent batches are at most 2; the 12-participant ceiling is a management review, not saturation or statistical proof. Each batch produces an information-power memo and qualitative value-of-information routing.

## 18. H2 preregistration gate

H2 is not created. A future H2 would be generated from qualifying cases, not validated by them, and must be tested on fresh holdouts. It cannot change technical fit or differentiation. A future corpus validator for cross-record controls is a disclosed hard blocker.

## 19. Outreach drafts

Supplier, buyer, organisation-level, follow-up, decline and withdrawal/deletion messages are drafted with placeholders and were not sent. They say research not sales, Regula not offered, no confidential information, no recording by default, voluntary participation and no compliance/market claim.

## 20. Validator and check results

Implementation commit: `1d0039f`. `validate_validation_readiness.py`: exit 0, 29 required files. Schema/examples: exit 0, 2/2 synthetic examples; both negative controls rejected. Six fast gates and `git diff --check`: exit 0 with disclosed inherited tree-guard warning. Custom runner: exit 0, 1389 passed. Pytest: exit 1, 1 failed and 2691 passed in 852.48 seconds. The same focused failure reproduces at untouched starting commit `799ce3d`; it is inherited count-literal collision debt in dated `08-EVIDENCE-RECONCILIATION.md`. Self-test: exit 0, 6/6. Doctor: exit 0, 8 pass and 4 info. Full decisive output is in `SESSION-LOG-DIRECT-TRANSACTION-2026-08-05-132655.md` and its tool transcript; no failure was hidden or suppressed.

## 21. Failed, abandoned and blocked attempts

The repository-wide pytest suite is not green due to the inherited collision above. No attempt was made to rewrite dated evidence, change the manifest, allowlist the file or weaken the test. H1 remains abandoned. Future H2 gating is blocked until the cross-record corpus validator exists and passes.

## 22. Shortcuts, assumptions, suppressions, skips, pins and exclusions

No test skip, xfail, allowlist, quarantine, dependency pin, hardcoded pass or swallowed error was introduced. No product suite result was inferred. The public organisation map was not expanded because it was already sufficient for this preparation decision. No real sample evidence exists.

## 23. Adversarial findings and dispositions

Reviewer A found seven substantive risks: participant-reported observability, conversion of pain into differentiation, generated/tested-case leakage, missing future corpus enforcement, empty gate fields, hypothetical-value coding and post-hoc negative-case handling. All except the deliberately disclosed corpus-validator blocker were corrected. Reviewer B found no material privacy/scope defect and required preservation of HOLD and all owner/professional gates. Verbatim reviews are in the permanent session log.

## 24. Hostile self-review

Every changed implementation file was reread. Strong-term hits were explicit stopped/unvalidated states, methodological cautions or source text. No unsupported result remained. No suppression keyword represented a newly introduced bypass.

## 25. Final verdict fields
...
Overall unit: PARTIAL because the required repository-wide pytest suite has one reproduced inherited failure. `READY_FOR_OWNER_REVIEW` describes the bounded launch pack only and does not authorise contact.

## 26. Complete open and partial ledger verbatim

The complete ledger follows verbatim so no open or partial item is summarised away.
...
## 27. Exact owner decisions required

Decide controller and research lead; approve the narrow purpose; obtain lawful-basis/data-protection and work-permission review; choose storage, access roles, retention and deletion; decide Stage A prohibitions; set zero/capped cost; and explicitly authorise contact only after those controls are complete.

## 28. Remaining uncertainty

No participant access, real transaction, recurrence, buyer acceptance, WTP, technical observability, technical efficacy or differentiation has been demonstrated. Cross-record corpus validation is not implemented. The inherited pytest collision remains.

## 29. Singular next action

Owner review of the decision register. Do not contact anyone until every required gate is settled and explicit contact authorisation is separately given.

## 30. Complete ready-to-run successor prompt

Review `/home/USER/getregula/docs/venture/direct-discovery-2026-08-05/05-OWNER-AND-PROFESSIONAL-DECISIONS.md` and the authoritative handover. Record owner decisions without contacting anyone. Obtain appropriately qualified review of controller/lawful-basis/data-protection and interviewer work-permission questions. Preserve H1 ABANDONED, H2 NOT CREATED, STOP/NOT APPROVED, external contact NOT AUTHORISED and real-data collection DISABLED. Do not launch discovery until all required gates are documented, storage is tested, participant information is reviewed, the cost boundary is approved and the owner separately authorises contact.
