...
`improvement/2026-08-programme` at `81e14a3`. `main` unchanged at `b5ac95c8`,
...
Per instruction, measured in a **detached worktree at commit `81e14a3`** with
...
"33 / 61 at `81e14a3`", never as a bare number.**

---

# CHECKPOINT, 29 July 2026, session 5d: ITEM 0, THREE GAPS SETTLED

Verification first, all at `b43f95d`: `git log --oneline main..HEAD | wc -l` = **58**;
`git status --porcelain` **empty**; `git rev-parse main origin/main` both
**b5ac95c8**; `pytest tests/ -q` **2439 passed rc=0** (932.48s);
...
`b43f95d`, 57 files scanned, 899 claims, **270 unsourced across exactly 20
...
Only `docs/improvement/STATE.md` differs between `81e14a3` and `b43f95d`
(MEASURED: `git diff --name-only 81e14a3 b43f95d` returns that one path), and
it moved 38 to 42 findings. So at `81e14a3` the buckets were **191 / 67 / 8 =
...
MEASURED by reverting the F32 repair in a scratch worktree at `b43f95d` and
...
The three paragraphs are the three source lines `81e14a3` added:
...
instead. Corpus excludes `docs/improvement/`, 156 tracked files, at `b43f95d`:
...
The evaluation froze product commit `140bdac` and preregistered at `f77473d`
before result execution. Two protocol deviations are explicit: `e7bb6d5`
separated protocol and product trees before results, and `f9e671e` supplied a
