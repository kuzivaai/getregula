...
**CORRECTION — the reconciliation table in commit `30cb981` is not
...
|---|---|---|
| F7 coordinates | `f1bf8c1` | fixture suite + line-count invariant |
| F6 percentages + fence + quarantine | `30cb981` | 3-case fence fixture + shrink-only ratchet |
| eli_data packaging | `00d3a5c` | verified against a real wheel build |
| **F1 count** | **`093b839`** | **10/10 captured runs at 1386/0/963** |
...
**Two `git add -A` deviations, disclosed in `093b839`:** the manifest and
its test landed in `bfd8657` instead of the count-correction commit; and
the F1 code landed in `0971e28`, whose message says F1 was NOT landed.
...
|---|---|---|
| S1 pre-F6, as landed at `f1bf8c1` | 182 | 0 |
...
records them: `main` at `6daacd2d` (identical to `origin/main`), tree
...
### F26 — the branch has been red since `9f9d876`. MAJOR, process.

MEASURED by worktree bisect, single test, three commits:

| Commit | Result |
|---|---|
| `47cc636` "record the full suite green after the F1 rebind" | **1 passed** |
| `9f9d876` corpus expansion 5 -> 30 | **1 failed** |
| `c3ce46c` HEAD at session start | **1 failed** |

`tests/test_classification.py::test_synthetic_fixture_perfect_precision_recall`
asserted `recall == 1.0`. `9f9d876` expanded the high-risk corpus from 5
