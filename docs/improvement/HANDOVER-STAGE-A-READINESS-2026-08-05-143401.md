# Regula Stage A activation-readiness handover

## 1. Objective and actual work

Closed the current-versus-historical count-literal collision class, corrected
the future research data/linkage architecture, and issued an owner activation
decision. No person was contacted, no real data collected, no money spent, and
no product, ownership, immigration, release or publication action occurred.

## 2. Sandbox, approval, network and Git state

Workspace `/home/USER/getregula`; workspace-write sandbox; restricted network;
unrestricted local verification approved where required. The escalated `.git`
probe passed. Repository and `.git` writes worked. Two read-only reviewers were
used. No concurrent Regula writer was identified.

## 3. Initial repository state

Initial HEAD `50b5a992b039b695fe39d39640fb6810576a686e`, tree
`de4cd0fc15213d3f49327b80c872e6645681316c`, clean worktree. `1d0039f`
existed and contained the prior launch pack.

## 4. Unsettled-claim reconciliation

The count failure reproduced at current pre-fix HEAD and untouched `799ce3d`.
The prior launch-pack commit did not introduce it. The readiness validator found
29 required files. Two schema examples were synthetic. Searches found no real
participant/contact/transaction corpus, later H1 reopening, H2 creation, or
external authorisation.

## 5. Exact failing test and starting-commit comparison

`test_count_literal_appears_nowhere_outside_the_manifest` failed at both trees,
naming `08-EVIDENCE-RECONCILIATION.md`. Both commands exited 1. The decisive
raw commands and outputs are recorded in the session log.

## 6. Current-versus-historical record analysis

The old guard mixed active claims and immutable dated evidence through broad
directory exclusions. The new boundary is centrally registered exact-path
`dated_evidence`, with date-in-path, full evidence commit, raw-byte hash and
commit/path/blob provenance. Current/generated surfaces cannot self-declare as
historical. Rename, deletion, Git failure and content drift fail closed.

## 7. Fail-before evidence

The new controls initially failed collection because `count_record_policy` did
not exist: exit 2. A separate revert control disabled broad-exclusion rejection;
the intended control failed, exit 1. Both are retained in the session log.

## 8. Class-wide implementation

Commit `a9cf58c` introduced the class. Reviewer findings then produced the final
class-wide correction in implementation commit
`688d1a7c2367328677f35f7efc8759184dc32f1b`, tree
`552752881d4417a616bee70595ee6f72049c9ea7`. The mechanism scans all tracked
decodable text irrespective of suffix, rejects read/discovery failure, hashes
bytes, and verifies immutable Git provenance. Historical records were not
rewritten.

## 9. Pass-after and revert controls

Focused policy suite: 21 passed, exit 0. Revert control failed as intended;
restoration returned the same focused suite to exit 0. Reviewer A found no
remaining material implementation defect.

## 10. Exact final full-suite output and exit status

Custom runner: 1,378 passed, 0 failed, 4 skipped, exit 0.

Restricted detached pytest: invalid environmental result—2 failures, 2,663
passes, 34 skips and 8 socket errors, exit 1. Unrestricted detached pytest:
2 failures, 2,671 passes, 34 skips, exit 1; both failures were the product's
canonical-root path safety rejecting `/tmp` fixture paths.

Controlling canonical-root unrestricted pytest, same exact commit/tree: two
thousand seven hundred seven passed, no failures or skips, exit 0, 24:06.
Pre/post tracked state was unchanged. Complete raw outputs are compressed under
`docs/improvement/evidence-stage-a-2026-08-05/`; SHA-256 values are recorded in
the session log and Git.

Self-test 6/6; doctor 8 pass/4 info; configured Ruff pass; 56 HTML files with
zero failures; security self-check zero unexpected; six fast gates pass; merge
blocker zero active-delivery findings. Exact commands and outputs are in the
session log. Historical tree-guard debt was printed, not suppressed.

## 11. Primary and peer-reviewed sources

The source table is in
`docs/venture/direct-discovery-2026-08-05/SOURCES.md`. It records retrieval on
2026-08-05, decisive passages, applicability and limits for ICO research,
lawful-basis/minimisation/transparency/anonymisation guidance (including the
under-review notice), Data (Use and Access) Act 2025 and commencement material,
MRS Code/qualitative guidance, GOV.UK work-permission guidance, Critical
Incident Technique, information power, SRQR, COREQ and negative-case methods.
None settles lawful basis or work permission.

## 12. Existing research-pack audit

Existing readiness materials were reused. Narrow corrections addressed the
synthetic schema, account/transaction linkage, participant information,
no-recording pathway, consent/data handling, organisation-map status and owner
gate. No parallel framework or validator was created.

## 13. Pseudonymisation assessment

Future analytical records are `PSEUDONYMISED`, not anonymous. Removing names
does not remove singling-out or linkage risk. Current examples are synthetic
only; no real-data handling is implemented or authorised.

## 14. Re-identification threat model

Threats cover rare role/sector combinations, event linkage, deterministic hash
attacks, insiders, Git, logs/backups, linkage disclosure, false matches,
counterpart inference, small cells, quoted free text, cross-session inference
and linked withdrawal. Likelihood is qualitative; residual decisions remain
owner/professional gated.

## 15. Contact, linkage and analytical architecture

Layer 1: identity/contact register in approved business storage, research-lead
access only, never Git. Layer 2: separately permissioned linkage register using
random participant/transaction tokens. Layer 3: pseudonymised analytical corpus
without identity/contact mapping. Withdrawal propagates through all linked
layers subject to the disclosed irreversible aggregation boundary.

## 16. Participant and privacy-material changes

Materials now state research-not-sales, no Regula offer, Stage A recording and
transcription prohibition, no confidential material, minimal personal-data use,
possible separately permissioned linkage, voluntary participation, stopping and
rights. They are drafts requiring legal/data-protection review, not approval.

## 17. Owner decision register

Owner defaults: GBP 0; recording, transcription, confidential documents,
repositories, code, security findings, special-category data, incentives,
Regula demonstration, sales, publication and real data in Git prohibited.
Still unresolved: controller, purpose, lead/interviewer, storage/jurisdiction,
processors, access roles, retention, deletion/privacy owners, lawful basis,
work permission and separate contact authorisation.

## 18. Work-permission status

`PROFESSIONAL_REVIEW_REQUIRED`. Ownership, title, unpaid status or general visa
guidance was not treated as a conclusion.

## 19. Organisation access-map status

`PREPARED_UNVALIDATED`. It contains organisation-level routes and role
archetypes, not names or personal contact data. It is not participant access.

## 20. Stage A gate

`HOLD`. The method is prepared, but controller, storage, lawful-processing and
interviewer work-permission decisions block activation. Contact remains not
authorised. The later Stage A design remains four transaction-qualified
participants (two supplier, two buyer/reviewer, at least two organisations),
no recording/documents/repos/demo/incentives/sales, and is only an access and
method test.

## 21. H2-validator deferral

H2 remains not created. The future cross-record validator is required only
after Stage A freezes a real-record schema and before H2 generation. Its future
contract covers uniqueness, range, linkage, negative cases, substitute basis,
generating/holdout separation and analyst independence.

## 22. Failed, abandoned and blocked attempts

- First pytest orchestration cell was lost at 77%; no exit, abandoned.
- Restricted detached rerun failed localhost sockets and canonical-root paths.
- Unrestricted detached rerun isolated the two canonical-root path failures.
- Canonical-root unrestricted rerun passed and controls the verdict.
- An over-broad Ruff sweep found 53 deliberate undefined names in synthetic
  fixtures; CI's configured exclusion controls, so it was not used as lint.
- No legal/work-permission conclusion was attempted.

## 23. Shortcuts, assumptions, suppressions and exclusions

No skip, xfail, allowlist, quarantine, pin, hardcoded pass or swallowed error was
introduced. Binary/NUL/non-UTF-8 tracked files are outside semantic prose
scanning; reviewer A classified this as a reasonable residual scope, not a text
exemption. CI's existing `tests/fixtures` Ruff exclusion was used exactly as
configured. No test result was cached or substituted.

## 24. Adversarial-review findings and dispositions

Reviewer A's suffix filtering, provenance, read-error, raw-hash and vacuity
findings were accepted and fixed. Reviewer B's transcript trigger and future
schema constraint findings were accepted and fixed. Both re-reviews reported
no remaining material defect. Their exact findings are in the session log.

## 25. Hostile self-review

All changed files were reread. Unsupported-term hits were either superseded
historical wording, explicit prohibitions, methodology limits or source titles.
The diff contained no new suppression mechanism. Claims were held to HOLD and
preparation-only status.

## 26. Final verdict fields

```text
REPOSITORY_SUITE: PASS
COUNT_LITERAL_COLLISION_CLASS: CLOSED
HISTORICAL_CURRENT_RECORD_BOUNDARY: ENFORCED
RESEARCH_DATA_CLASSIFICATION: PSEUDONYMISED
REIDENTIFICATION_THREAT_MODEL: COMPLETE
CONTACT_IDENTITY_SEPARATION: DESIGNED
TRANSACTION_LINKAGE_SEPARATION: DESIGNED
...
## 27. Complete open and partial ledger verbatim

The complete ledger follows verbatim between the markers.

<!-- BEGIN VERBATIM LEDGER -->
# Open-items ledger
...
Name the controller, research purpose, research lead and interviewer; approve
the storage system/jurisdiction/processors/access roles; set retention and
deletion/privacy ownership; obtain qualified lawful-basis and work-permission
review; then issue a separate explicit contact authorisation. Until all are
resolved, HOLD.

## 29. Remaining uncertainty

No participant access, recurrence, transaction prevalence, buyer acceptance,
willingness to pay, technical efficacy or differentiation has been established.
The future schema and operating controls have not handled real variation. The
three-layer architecture is designed, not deployed or professionally approved.

## 30. Singular next action

Owner obtains one narrowly scoped qualified review of controller/lawful-basis,
storage/retention and the named interviewer's work-permission boundary, then
records the unresolved owner decisions. Do not contact anyone before a later
explicit authorisation.

## 31. Ready-to-run successor prompt

```text
Repository: /home/USER/getregula

Objective: resolve only the owner/professional Stage A activation gates recorded
in N69/N71 and HANDOVER-STAGE-A-READINESS-2026-08-05-143401.md. Preserve STOP,
NOT_APPROVED, H1 ABANDONED, H2 NOT CREATED, EXTERNAL_CONTACT NOT_AUTHORISED and
REAL_DATA_COLLECTION DISABLED. Reproduce commit/tree and read the full ledger.
Record qualified written conclusions for controller/lawful basis and interviewer
work permission; record owner decisions for purpose, lead, approved storage,
jurisdiction/processors/access, retention, deletion/privacy ownership and the
GBP 0/prohibition defaults. Do not contact anyone, collect data, appoint an
adviser, spend money, build product, publish, push or release. If any blocking
decision remains unresolved, return STAGE_A_PACK: HOLD. Only prepare a separate
owner contact-authorisation decision after every prerequisite is evidenced.
```
