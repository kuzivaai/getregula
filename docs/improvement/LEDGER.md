...
`9ed56ec` and revised by `7a7a4c2`. **Provenance corrected 2026-08-18:** this
entry previously named `7a7a4c2` and `ea64ffe`. Verified against the full file
list of each commit, `ea64ffe` does not touch `site/pricing.html`; it
...
prices on the pricing page is `9ed56ec`. The open question is therefore not
...
fixture.** The before-arm is a worktree at `23195b5^` (v6, no params token); the
...
commits are on the remote and in PR #55, and everything after `3f52501` is
unpushed.**
...
it existed: the real figures at that commit are `main..3f52501 = 2`,
`3f52501..HEAD = 39`, `main..HEAD = 41`. **This is the self-referential
...
and neither has been exercised on the commits after `3f52501`, which carry the
decision kernel, the claim closures, the fact loop and the cache repairs. **The
gap is real, it is everything after `3f52501`, and it is not zero.**
...
reconciliation: 16 published = 14 tagged + 2 untagged
  ...
  1.5.1  uploaded 2026-04-04T19:47:02  changelog=NO   version-commit=c598fd0
      artefact: regula_ai-1.5.1-py3-none-any.whl
      commit  : c598fd0 2026-04-04 release: v1.5.1 - fix stale PyPI package,
                add --version flag, connect domain
  1.7.2  uploaded 2026-06-15T23:13:44  changelog=yes  version-commit=67bba4c
      artefact: regula_ai-1.7.2-py3-none-any.whl, regula_ai-1.7.2.tar.gz
      commit  : 67bba4c 2026-06-16 chore(release): v1.7.2 - MCP Registry
