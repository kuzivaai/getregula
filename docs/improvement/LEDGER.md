...
`74a8567` changed five files under `site/`, moving four `lastmod` values, so
the committed sitemap no longer matched a fresh run. `74a8567` was incomplete
...
`from scripts.validate_validation_readiness import ...` since `ddbea55` and
...
status with a default budget of 600000ms. The deploy job for `80d79a3`
...
the push of `b978076`. `pages.yml` is now workflow_dispatch only, leaving one
...
`main` was fast-forwarded to `b978076` with all 24 CI checks passing, so
...
**Measured, not assumed.** Four deployment attempts for commit `b978076`, all
...
```
HttpError: Deployment request failed for 3bc1aeb... due to in progress
deployment. Please cancel b978076... first or wait for it to complete.
```

A deployment for `b978076` is stuck in-progress server-side and blocks every
subsequent deployment of any commit. **The stuck deployment cannot be cleared
from here.** `POST /repos/kuzivaai/getregula/pages/deployments/b978076.../cancel`
returns `HTTP 204 No Content` and
`GET .../pages/deployments/b978076...` then reports
...
**Fail-before.** The full suite at `eb3fef3` (tree `53f7e29`, quiescent
...
`203e03b3d9c63ff45b22df3fc18f8cb480e63d1f`, base tree
...
commit=203e03b3d9c63ff45b22df3fc18f8cb480e63d1f
...
The clean custom run at commit `f8864cb30aa721e5b402ca32cd35f5577974896f`
...
`ba5c033123658b5160d97c234b88772d4fd918ac` returned 1 and exposed a separate
...
`b6a151f5995d8ea48d1af024c8f0ca7333346ff4`, tree
...
`b14997e6f462fea976fc96eafffe2cec13ca6b4f`, tree
...
Commit `HELD:9e21501`, held on `feat/engagement-fixes`; nothing is on main.
