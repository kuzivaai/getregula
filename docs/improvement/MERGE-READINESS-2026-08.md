...
1. **CI has run on the PR head and on nothing after it.** All four Python
   versions and all ten composite-action jobs passed on `238d1f1`. Everything
   after `238d1f1` carries the decision kernel, the claim closures, the fact
   ...
   erratum 5. Re-derive with `git rev-list --count 238d1f1..HEAD`.)
...
exercised on the commits after `238d1f1`, which carry the decision kernel, the
claim closures, the fact loop and the cache repairs. **The gap is everything
after `238d1f1`**, and everything that makes this branch worth merging is inside
it.
...
Measured at `dac9a903`, tree `dea0e2d2`. Every figure below was produced by a
...
- The push is a **fast-forward**: `238d1f1` is an ancestor of `dac9a903`,
...
- It changes **195 files**, of which **57 are under `site/`**, the directory
  ...
  `git diff --name-only 238d1f1..HEAD`, the second figure by filtering that
...
Introduced by `5786666`, revised by `b349531`; neither is an ancestor of the PR
...
Before-arm is a worktree at `d32c7be^` (v6); after-arm is the tip (v7).
...
particular **no clean baseline was established for the previous tip `188ddae`**;
the run started for that purpose was invalidated by edits made while it ran, and
the figures above are for `6acd7b8` only.
