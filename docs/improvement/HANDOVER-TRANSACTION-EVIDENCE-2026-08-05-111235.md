# Regula public transaction-evidence handover

Date: 2026-08-05
Session: 111235 Europe/London
Repository: `/home/USER/getregula`

## 1. Objective and actual work

The unit reconciled validation-readiness evidence, preregistered and executed a
public falsification search for the exact UK general-insurer claims-triage
supplier-onboarding hypothesis, adversarially reviewed the result, updated
continuity and stopped without external action.

## 2. Sandbox, approval, network and Git state

Workspace writes were available. Default `.git` writes failed; the user-approved
minimum unrestricted Git probe and scoped commits succeeded. Shell network was
restricted; public read-only web retrieval was used. Two subagents performed
read-only reviews. No concurrent repository writer was authorised.

## 3. Initial repository state

Initial HEAD `55cec556440e5130c700cc00f662838ff84f1bb4`, tree
`d83bf6973e34f5b0da611896884a51b0a5792da6`, branch
`audit/regulatory-current-2026-08-04`, initially clean and six commits ahead of
its remote tracking branch.

## 4. Raw evidence reconciliation

Implementation commit `f72f2f836cd3b4a5ea8c7c9cf88763a1c0fe47bb`, tree
`563876fefcfd698bd5e64d4ac8bf87c9d5172f40`, exists. It contains the 29-file
validation-readiness pack and its 23-source register. Exact clean-tree reruns:

- custom runner: exit 0; 1,378 passed, 0 failed, 4 skipped;
- pytest `-q -rs`: exit 0; 2,658 passed, 34 skipped in 857.65 seconds;
- self-test 6/6; doctor 9 pass and 3 information results;
- pack validator, mutation controls, configured ruff, HTML parser, security
  self-check, six fast gates and claim diff: exit 0.

All 38 skip instances concern absent local hook files and are itemised in the
raw output at `SESSION-LOG-TRANSACTION-EVIDENCE-2026-08-05-111235.md`. Historical
fail-before detail and original reviewer transport remain partial.

## 5. N64/N65 disposition

N65's corrected merge-claim predicate is implemented and exact-tree verified.
N64 remains partial because wider release blockers remain. Prior 96/538,
474/463 and 45-test figures lack complete retained raw output and are labelled
prior-record summaries. No release-readiness conclusion follows.

## 6. Research protocol

`docs/venture/transaction-evidence-2026-08-05/PROTOCOL.md` froze H1, H0, 14
questions, evidence classes and stopping rule before searches. H1 was not
broadened. No challenger was selected.

## 7. Complete source and query register

`docs/venture/transaction-evidence-2026-08-05/SEARCH-REGISTER.md` contains all 13
query rounds, 231 returned/screened results, 17 unique retained sources,
exclusion classes, URLs, cut-off handling and protocol deviations.

## 8. Search limitations

Rounds 1–6 lack second-level timestamps. The interface did not expose stable raw
responses for content hashing or universe hit counts. Early gap rankings were
not durably logged after every round. Undated current pages are non-controlling
for the 4 August cut-off. Exclusions are category-mapped rather than URL-listed
one by one. Public absence is not market absence.

## 9. Direct transaction evidence

No public artefact demonstrated the joined H1 transaction. Dated evidence
separately establishes claims-AI use and general supplier onboarding. Admiral's
live policy explicitly describes AI-supplier due diligence, but its exact cut-off
version is unresolved.

## 10. Buyer requirements

Public buyer material supports general risk-based onboarding, security, data
protection, continuity and governance requirements. The current Admiral page adds
AI risk management, accountability and incident controls. Mandatory H1 evidence,
format and acceptance criteria remain unknown.

## 11. Supplier burden

No qualifying small UK supplier publicly described H1 rework, delay, rejection
or cost. Recurring questionnaires imply activity but do not quantify burden.

## 12. Budget and decision roles

Possible functional roles are visible, but requester, reviewer, blocker,
approver and budget owner for H1 were not demonstrated. Technology purchases do
not establish willingness to pay for Regula.

## 13. Regula relevance

No real buyer questionnaire or permissioned evidence set exists, so no defensible
proportion of technically observable evidence can be calculated. Organisational,
contractual, legal, security, operational and independent evidence dominates
many public requirement classes.

## 14. Existing substitutes

General procurement, third-party-risk, questionnaire, consulting, legal,
certification, testing and assurance routes exist. Their adequacy for H1 is not
demonstrated, and Regula's superiority or replication difficulty is not
demonstrated.

## 15. Access feasibility

`WEAK`. Public directories and events expose populations, not permission,
representativeness, buyer access or willingness to participate.

## 16. Contrary and null findings

No source disproved H1. No public insurer-issued H1 questionnaire, matching small
supplier burden account, transaction cost, budget owner or source-code request
was found. This is insufficient exact evidence, not proof of nonexistence.

## 17. Hypothesis verdict

`PUBLIC_TRANSACTION_EVIDENCE: GENERAL_PROBLEM_ONLY`.
`HYPOTHESIS_STATUS: ABANDON` as the governing beachhead. No adjacent challenger
was sufficiently supported.

## 18. Strongest case against

Private insurer procurement is rarely public, and the separate public components
make a confidential matching transaction plausible. Controlled conversations
could reveal unavailable evidence. That possibility does not demonstrate burden,
budget, acceptance or Regula relevance and does not justify paid review now.

## 19. Owner recommendation

`OWNER_SPEND_RECOMMENDATION: DO_NOT_SPEND` is a reasoned management judgment:
paid review is premature without a matching transaction and Regula-relevant
evidence. `CONTACT_READINESS: HOLD`. Preserve the record; do not broaden H1.

## 20. Failed, abandoned or blocked attempts

- Default `.git` probe failed exit 73; approved escalation succeeded.
- The first focused pytest command used two nonexistent filenames and exited 4;
  the correct files then passed 15/15.
- The initial stopping-rule assertion was invalid; three more rounds were run.
- Initial search arithmetic said 193; corrected total is 231.
- A retired Direct Line URL redirects to Aviva; the dated annual report replaced
  it as canonical evidence.
- The FCA live page failed one direct open and remains current-only/unresolved.

## 21. Shortcuts, assumptions, suppressions, skips, pins and exclusions

No test suppression, skip addition, allowlist, quarantine, dependency pin or
hardcoded pass was introduced. Search audit limitations are in section 8. The
full product suite was not rerun after documentation-only edits because its exact
implementation-tree raw evidence was reproduced during this unit; focused docs,
continuity and all required fast gates were run afterward. Tree-guard baseline-
drift warnings were retained and not treated as failures because each gate exited
0. No product or public surface changed.

## 22. Adversarial-review findings and dispositions

Reviewer A found the stopping-rule, arithmetic, source-class, cut-off,
replication, access and auditability defects. All material findings were fixed;
final verdict: no remaining material blocker. Reviewer B found overstrong
handover evidence, continuity conflict and unretained historical metrics. They
were downgraded or reconciled; final verdict: no remaining material blocker.

## 23. Verification commands, outputs and exits

- `git diff --check`: exit 0.
- `python3 scripts/validate_validation_readiness.py`: exit 0, 29 required files.
- `python3 -m pytest tests/test_validation_readiness.py tests/test_ledger_status.py -q`: exit 0, 15 passed.
- `python3 scripts/claim_auditor.py --verify-facts`: exit 0, 148 references/17 files.
- `python3 scripts/site_integrity.py`: exit 0, result OK with documented warnings.
- `python3 scripts/cascade_count.py --check`: exit 0, 2,692/11.
- recall and gap checks: exit 0, fresh matches.
- self-reference controls: exit 0, negative rejected and positive accepted.
- `python3 scripts/claim_auditor.py --diff-base main`: exit 0, 38 files/378 claims/0 unsourced.

Complete custom-runner and pytest outputs, every skip reason, hashes and current
gate summaries are in the 1,002-line permanent session log.
...
REGULA_ASSISTED_DELIVERY: STOP
SOFTWARE_BUILD: STOP
```

## 25. Complete ledger verbatim

The complete ledger follows verbatim so all open and partial items, stable IDs,
first-raised dates and statuses are retained rather than summarised.
...
All external contact, data collection, recordings, document or repository receipt, spend, adviser engagement, pilot, delivery, product build, ownership, founder, immigration, publication, push and deployment actions remain unauthorised.

## 27. Remaining uncertainty

The exact transaction may exist privately. Its frequency, burden, decision authority, budget, evidence content, accessibility and Regula relevance remain unknown. The historical fail-before error composition and original reviewer transport are only partial. Current-only web sources are not proven at the cut-off version.

## 28. Singular next unit

No automatic commercial unit is recommended. Only if separately authorised, conduct an owner-only inventory of direct non-Regula market evidence already lawfully possessed, without importing personal or confidential data into this repository, to decide whether any different hypothesis deserves preregistration.

## 29. Ready-to-run successor prompt

```text
Work in /home/USER/getregula. Read AGENTS.md, the complete LEDGER.md, docs/venture/transaction-evidence-2026-08-05/PROTOCOL.md, SEARCH-REGISTER.md, PUBLIC_TRANSACTION_EVIDENCE_REVIEW.md, and this handover. Preserve VENTURE_DECISION: STOP, PRODUCT_PILOT_STATUS: NOT_APPROVED, REAL_DATA_COLLECTION: DISABLED, EXTERNAL_CONTACT: NOT_AUTHORISED, REGULA_ASSISTED_DELIVERY: STOP and SOFTWARE_BUILD: STOP. Do not contact anyone, spend money, collect data, alter product/public surfaces, or broaden abandoned H1. Recompute Git state. The only permitted next decision is whether lawfully held owner-only, non-personal, non-confidential evidence already available outside this repository justifies preregistering one different exact transaction hypothesis. Do not copy that evidence into the repository without explicit scope and data review. If no such evidence is identified by the owner, record no successor commercial unit and stop. Use at most one adjacent hypothesis, label every inference, update the ledger, run documentation/continuity checks, commit evidence only, and write an authoritative handover.
```
