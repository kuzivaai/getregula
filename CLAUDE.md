...
- **59 CLI commands** (verified via `regula --help-all`; `regula -h` shows 7 primary commands via progressive disclosure)
...
- **1,199 tests** (all passing — verified via `python3 -m pytest tests/ --collect-only -q` on 2026-04-19)
...
# Run all four sequentially before claiming anything is "done"
...
2. The custom runner walks `globals()` of `tests/test_classification.py` and runs every `test_*` function it finds (auto-discovery). Pytest discovers separate `tests/test_*.py` files natively.
...
Source of truth: `scripts/constants.py:VERSION` (imported by `cli.py`)
