...
Extends the existing two-rater Cohen's kappa tooling
(benchmarks/compute_kappa.py) to N >= 3 independent raters with Fleiss'
kappa, which is the statistic the corpus upgrade targets (>= 3 labellers,
Fleiss' kappa >= 0.7; see benchmarks/MULTI_ANNOTATOR_PROTOCOL.md).
...
def interpret(kappa) -> str:
    """Publication guidance per MULTI_ANNOTATOR_PROTOCOL.md thresholds."""
    if kappa is None:
        return "undefined (zero label variance)"
    if kappa >= 0.7:
        return "meets the >=0.7 target: publishable as-is"
    if kappa >= 0.6:
        return "0.6-0.7: publishable with disclosure of ambiguity analysis"
    return ("<0.6: itself a publishable finding about task subjectivity; "
            "report per-tier breakdown and the disagreement dossier")
...
def main() -> int:
    ...
    result = fleiss_kappa(raters)
    print(json.dumps(result, indent=2))
    if "kappa" in result:
        print(f"\nInterpretation: {interpret(result['kappa'])}")
    return 0
