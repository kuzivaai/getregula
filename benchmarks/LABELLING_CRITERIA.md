...
7. Set `"labeller"` to a non-identifying study ID (for example,
   `"rater-01"`) so ratings can be linked without publishing personal data.
8. Use `"uncertain"` when the evidence supports competing interpretations and
   `"not_assessable"` when required project or deployment context is absent.

## Quality Controls

- **Rater linkage**: Every entry has a non-identifying `"labeller"` study ID.
  The current corpus uses one maintainer rater and is not independent ground
  truth.
- **Independent replication**: Claims about real-world accuracy require the
  blinded multi-rater protocol in `MULTI_ANNOTATOR_PROTOCOL.md`.
...
- **Rater 1**: a maintainer label retained as a provenance record.
- **Raters 2 and 3+**: independent humans with sufficient technical and domain
  background for the assigned strata. Their qualifications, conflicts and
  recusals are recorded without publishing personal data.
...
4. Nominal Krippendorff alpha with an item-bootstrap confidence interval is
   computed via `benchmarks/annotation_stats.py`; pairwise Cohen kappa is only a
   supplementary view.
...
- Non-identifying rater study IDs and qualification/conflict summaries

### Interpretation

No universal agreement band is a publication gate. Report the estimate and
bootstrap interval together with label prevalence, missingness, raw agreement
and disagreement reasons. Any codebook revision is developed on a training
subset and evaluated on a fresh held-out subset.

### Timing gate

No precision figure from the targeted corpus is publishable until:
1. Both raters have labelled their full assigned sets
2. Agreement, its interval, missingness and disagreements are reported
