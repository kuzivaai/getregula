...
2. **No head-to-head has been run before** (verified 27 Jul 2026: no
   published comparison found; the business dossier's known-gaps list
   agrees). This study aims to be the first and to be citable by anyone,
...
6. **Ground truth.** The multi-annotator corpus (MULTI_ANNOTATOR_PROTOCOL.md).
   ...
   import exactly the weakness the upgrade exists to fix. (Rater 2
   recruitment is the owner-action bottleneck.)
...
1. Ground-truth gate: rater 2 not yet recruited (owner action).
