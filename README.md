...
[![Tests](https://img.shields.io/badge/tests-1232%20passing-brightgreen.svg)](#verified-numbers)
...
- **False positives will occur.** Blind-labelled benchmark on 50 randomly selected Python AI repos measured **83.5% precision on production code** (N=115, default `--skip-tests` settings). Per-tier: `ai_security` (85%), `agent_autonomy` (83%), `limited_risk` (88%), `minimal_risk` (100%). The `high_risk` tier (33%) remains weakest — domain keyword findings now require `--domain` declaration or import fingerprinting (v1.7.0+). Full methodology, corpus selection, and reproduction steps: [`benchmarks/README.md`](benchmarks/README.md).
