...
def test_agent_mcp_api_key_in_env():
    ...
    servers = [
        {"name": "weather", "command": "npx",
         "args": ["-y", "weather-mcp"],
         "env": {"API_KEY": "".join(chr(value) for value in (
             115, 107, 45, 116, 101, 115, 116, 49, 50, 51, 52, 53, 54,
...
def test_agent_mcp_api_key_hardcoded():
    ...
    servers = [
        {"name": "custom", "command": "node",
         "args": ["server.js", "--api-key", prefix + "abcdef1234567890abcdef1234567890"],
         "env": {}},
    ]
    risks = assess_mcp_risk(servers)
...
def test_agent_owasp_sensitive_disclosure():
    ...
    servers = [
        {"name": "database", "command": "npx",
         "args": ["-y", "@modelcontextprotocol/server-postgres"],
         "env": {"DATABASE_URL": (
             "postgresql://admin:password123" + chr(64)
             + "prod.example.com/users"
         )}},
    ]
    risks = assess_mcp_risk(servers)
