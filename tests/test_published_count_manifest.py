...
def _count_pattern(count: int):
    ...
    silent drift `AGENTS.md` forbids outright. It now
...
class TestPublishedCountManifest(unittest.TestCase):
    ...
    def test_trust_publishes_the_custom_runners_own_function_count(self):
        ...
        `AGENTS.md` requires, moved the runner from 963 functions
        ...
        table), and this guard covers BOTH. Other tracked hits can include
        `CHANGELOG.md` and code comments that record what was true on a past
        date, plus hash
