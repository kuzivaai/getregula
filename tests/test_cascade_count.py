...
class TestEveryPublishedSurfaceCarriesTheCanonicalCount(unittest.TestCase):
    ...
    # Historical and verbatim records must keep the number that was true when
    # written; rewriting them would falsify the record. Everything else
...
class TestTheCommandCountIsCascadedInThreeLanguages(unittest.TestCase):
    ...
    the prior audit recorded the command count as ungated: `claim_auditor
    ...
    the number and its unit word. That is the prior audit's finding (the unit word is
