...
def test_sibling_importers_have_path_insert():
    ...
    with sys.path.insert (any form), per AGENTS.md.
...
def test_no_tracked_python_imports_the_scripts_package():
    ...
    `from scripts.validate_validation_readiness import ...` since `268937a`
    ...
    elsewhere. `AGENTS.md` already required bare
...
def test_the_scripts_package_import_scan_is_not_vacuous():
    ...
    assert "tests/test_public_repo_guard.py" in files, (
        "the sweep does not reach a current test module")
