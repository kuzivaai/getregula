...
match as one of Article 11's four components.
...
def test_clean_fixture_reports_nothing():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        found = tracked_inputs.untracked_inputs(fixture, root=root)
...
def test_restoring_the_directory_clears_the_report():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert tracked_inputs.untracked_inputs(fixture, root=root) != []
        stray.unlink()
        assert tracked_inputs.untracked_inputs(fixture, root=root) == [], \
...
def test_assert_inputs_tracked_raises_with_the_paths_named():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        (fixture / "local-only.yaml").write_text("key: value\n")
        try:
            tracked_inputs.assert_inputs_tracked(fixture, root=root)
        except tracked_inputs.UntrackedInputError as exc:
...
def test_nonexistent_target_raises_rather_than_passing_silently():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            tracked_inputs.untracked_inputs(
                root / "fixtures" / "typo", root=root
            )
...
def _clone_with_working_tree_scripts(repo, dest):
    ...
    for rel in ("scripts/tracked_inputs.py", "scripts/build_gap_demo.py",
