...
reader and none of these checks is a substitute for one; that gap is recorded in
docs/ux/USERS-JOURNEYS-IA-2026-08.md. These are mechanical properties only:
...
from a list in this file (measurement rule 4c in .claude/rules/measurement.md).
...
def test_every_regulatory_date_matches_the_omnibus_module():
    """scripts/omnibus.py is the single source of truth (CLAUDE.md)."""
    expected = {
...
def test_the_shared_link_encoding_is_versioned():
    ...
    .claude/rules/quality-standards.md: old encodings must keep decoding
