...
commits and a finish at `41feb51` while its own itemisation listed six.
`git rev-list --count 9e6b6de..41feb51` is 4, so the arithmetic was
internally consistent; the **declared finish was stale**, because `190da47`
and `ef2b8de` landed after the header was written. No arithmetic check
...
def test_commit_anchors_reconciles_the_count_against_git(tmp_path):
    ...
    real = subprocess.run(
        ["git", "-C", str(REPO_ROOT), "rev-list", "--count", "9e6b6de..41feb51"],
