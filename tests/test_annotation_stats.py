...
class TestFleissKappa(unittest.TestCase):
    ...
    def test_interpret_bands(self):
        self.assertIn("without a universal", annotation_stats.interpret(0.75))
        self.assertIn("without a universal", annotation_stats.interpret(0.4))
...
def test_krippendorff_alpha_nominal_handles_missing_ratings():
    """Hand-computed nominal alpha: Do=2/7, De=4/7, alpha=0.5."""
    r1 = {"a": "tp", "b": "tp", "c": "fp"}
    r2 = {"a": "tp", "b": "fp"}
    r3 = {"a": "tp", "c": "fp"}
    result = annotation_stats.krippendorff_alpha_nominal(
        [r1, r2, r3], n_resamples=0
    )
    assert result["alpha"] == 0.5
    assert result["n_items"] == 3
    assert result["missing_ratings"] == 2
    assert result["rating_coverage"] == 0.7778


def test_krippendorff_alpha_bootstrap_is_deterministic():
    raters = [
        {"a": "tp", "b": "tp", "c": "fp", "d": "fp"},
        {"a": "tp", "b": "fp", "c": "fp", "d": "fp"},
        {"a": "tp", "b": "tp", "c": "tp", "d": "fp"},
    ]
    first = annotation_stats.krippendorff_alpha_nominal(
        raters, n_resamples=200, seed=17
    )
    second = annotation_stats.krippendorff_alpha_nominal(
        raters, n_resamples=200, seed=17
    )
    assert first == second
    assert first["alpha_ci_95"] is not None
