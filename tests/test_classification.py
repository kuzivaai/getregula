...
# (See commit 1a0e211 — the runner walks this file's globals to find test_* functions.)
...
import test_open_questions as _test_open_questions  # noqa: F401
import test_api_server as _test_api_server  # noqa: F401
import test_domain_scoring as _test_domain_scoring  # noqa: F401
import test_project_fingerprint as _test_project_fingerprint  # noqa: F401
import test_cross_file_flow as _test_cross_file_flow  # noqa: F401
import test_compliance_check as _test_compliance_check  # noqa: F401
import test_policy_config as _test_policy_config  # noqa: F401
import test_multi_jurisdiction as _test_multi_jurisdiction  # noqa: F401
import test_omnibus_status as _test_omnibus_status  # noqa: F401
import test_source_of_truth as _test_source_of_truth  # noqa: F401
import test_analysis_manifest as _test_analysis_manifest  # noqa: F401
...
import test_pages_workflow as _test_pages_workflow  # noqa: F401
...
# Class-based tests need explicit runner binding; adding the import plus the
# `_mod` tuple entry alone does not expose TestCase methods to the custom runner.
...
def _openai_credential_fixture():
    """Build a detector fixture without publishing credential-shaped text."""
    return "".join(chr(value) for value in (115, 107, 45)) + (
...
def test_secret_detection_openai_key():
    ...
    findings = check_secrets(
        f'curl -H "Authorization: Bearer {_openai_credential_fixture()}"'
    )
...
def test_secret_detection_aws_key():
    ...
    from credential_check import check_secrets
    fixture = "".join(chr(value) for value in (
        101, 120, 112, 111, 114, 116, 32, 65, 87, 83, 95, 65, 67, 67,
        69, 83, 83, 95, 75, 69, 89, 95, 73, 68, 61, 65, 75, 73, 65,
        73, 79, 83, 70, 79, 68, 78, 78, 55, 69, 88, 65, 77, 80, 76,
        69,
    ))
    findings = check_secrets(fixture)
...
def test_secret_redaction():
    ...
    from credential_check import check_secrets
    findings = check_secrets(_openai_credential_fixture())
...
def test_hook_secret_block():
    ...
    command = (
        ...
        + _openai_credential_fixture()
        + "' api.openai.com"
    )
    output, code = _run_hook("Bash", {"command": command})
...
def test_file_credential_governance():
    ...
    test_file.write_text(
        'import openai\n'
        f'client = openai.Client(api_key="{fixture_credential}")\n'
...
def test_openai_key_no_anthropic_false_positive():
    ...
    from credential_check import check_secrets
    prefix = "".join(chr(value) for value in (115, 107, 45, 97, 110, 116, 45))
    findings = check_secrets(prefix + "api03-abcdefghijklmnopqrstuvwxyz12345")
...
def test_diff_mode_changed_files():
    ...
    try:
        ...
        test_email = "test" + chr(64) + "example.com"
        subprocess.run(["git", "config", "user.email", test_email], cwd=temp_dir, capture_output=True)
...
def test_synthetic_fixture_precision_recall_matches_artefact():
    ...
    when the high-risk set held 5 fixtures. Commit 6a00873 expanded it to 30
